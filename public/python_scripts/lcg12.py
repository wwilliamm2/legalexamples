...
then get the response and pass that response re.match().
...
# Pass the response to re.match()
rsp_s = str(response)
# nope pttrn_r = r'(.*GenerateContentResponse.*)(text)(.*=.*)'
# pttrn_r = r'(.*candidates.*)(text=)(.*)(role=.*model)(.*$)'
# pttrn_r = r'(.*candidates.*)(text=)(.*\")(role=.*model)(.*$)'
# pttrn_r = r'(.*candidates.*)(text=)(".*)(role=.*model)(.*$)'
# pttrn_r = r'(.*candidates.*)(text=)(")(.*)(")(.*role=.*model)(.*$)'
pttrn_r = r'(^.*text=)(")(.*)(")(.*$)' # works good
mtch = re.match(pttrn_r, rsp_s)
text_s = mtch[3]
print(text_s)
