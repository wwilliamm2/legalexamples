'''~/lx/lx14/public/py/llm_groq4bash17.py'''
...
. gemini2.bash
pip install groq
python ~/lx/lx14/public/py/llm_groq4bash17.py
* support cmd-line tokens like this:
python llm_groq4bash17.py ~/prompt.txt 
python llm_groq4bash17.py ~/prompt.txt gemma2-9b-it
python -i llm_groq4bash17.py ~/prompt.txt gemma2-9b-it
...
for attempt in range(4):
    try:
        ...
        e2_s = f"Failed to retrieve chat completion."
        os.system('mkdir -p /tmp/groq_usage_info/')
        n_s = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
        with open(f'/tmp/groq_usage_info/groq_{n_s}.txt','w') as gf:
            gf.write(f"{e1_s}\n{e2_s}")
